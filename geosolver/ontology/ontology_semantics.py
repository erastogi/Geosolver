...
def _polygon_to_points(polygon) :
    return [Point(point[0] , point[1]) for point in polygon]
...
############################################### Change MeasureOf, LengthOf, Measures ###########################
def congruent_triangle(triangle1, triangle2) :
    #returns if two triangles are congruent
    lines1 = _polygon_to_lines(triangle1)
    lines2 = _polygon_to_lines(triangle2)
    
    conditions_sas = list()
    conditions_aas = list()
    conditions_asa = list()
    #SSS
    out1 = reduce(operator.__and__, (Equals(LengthOf(line1) , LengthOf(lines2[i])) for i,line1 in enumerate(lines1)), True)
    for i , b  in enumerate(lines1) :
        a = lines1[i-1]        
        #ASA
        conditions_asa.append(Equals(LengthOf(a) , LengthOf(lines2[i-1])) and  
        Equals(MeasureOf(Angle(b[1], a[0],a[1])) , MeasureOf(Angle(lines2[lines1.index(b)][1] , lines2[i-1][0] , lines2[i-1][1]))) and
        Equals(MeasureOf(Angle(b[1],a[1],a[0])) , MeasureOf(Angle(lines2[lines1.index(b)][1], lines2[i-1][1], lines2[i-1][0] ))))                       
        

        #SAS        
        conditions_sas.append(Equals(LengthOf(a) , LengthOf(lines2[i-1])) and 
                   Equals(LengthOf(b) , LengthOf(lines2[lines1.index(b)])) and 
        Equals(MeasureOf(Angle(a[0],a[1],b[1])) , MeasureOf(Angle(lines2[i-1][0],lines2[i-1][1],lines2[lines1.index(b)][1]))))
    
    
        #AAS
        conditions_asa.append(Equals(LengthOf(a) , LengthOf(lines2[i-1])) and  
        Equals(MeasureOf(Angle(a[0],b[1],a[1])) , MeasureOf(Angle(lines2[i-1][0], lines2[lines1.index(b)][1] , lines2[i-1][1] ))) and
        Equals(MeasureOf(Angle(b[1],a[1],b[0])) , MeasureOf(Angle(lines2[lines1.index(b)][1], lines2[i-1][1], lines2[i-1][0])))) 
    
    
    
    out3 = reduce(operator.__or__, ( condition  for condition in conditions_sas), False)
    out2 = reduce(operator.__or__, ( condition  for condition in conditions_asa), False)
    out4 = reduce(operator.__or__, ( condition  for condition in conditions_aas), False)
    out =  reduce(operator.__or__, ( res for res in [ out1 ,out2,out3,out4] ) , False)
    return out
 
    
def vertically_opposite_angles(angle1, angle2) :
    #returns if two angles are vertically opposite or not
    out1 = ( PointLiesOnLine(angle1[1] , Line(angle1[0] , angle2[2])) & PointLiesOnLine(angle1[1] , Line(angle1[2] , angle2[0])) ) 
    out2 = ( PointLiesOnLine(angle1[1] , Line(angle1[0] , angle2[0])) &  PointLiesOnLine(angle1[1] , Line(angle1[2] , angle2[2])) )
    return out1 | out2

def angle_sum_property(triangle) :
    #returns the value of third angle
    angles = _polygon_to_angles(triangle)
    
def exterior_angle(triangle , line ) : 
    #returns the value of exterior_angle
    points = _polygon_to_points(triangle)
    distant_point = line[0] 
    if distant_point in points :
        distant_point = line[1]
    for point in points :
        if PointLiesOnLine(point,line).conf > 0.95 :
                first_point = set(points) - set(line[0] , line[1] , point)
                ext_angle = Angle(first_point ,point,distant_point)    
                #assign value to exterior angle
                second_point = set(line[0] , line[1]) - distant_point
                val = MeasureOf(Angle(second_point, first_point ,point) ) +  MeasureOf(Angle(first_point, second_point ,point) )
                ### correct the return part
                return  Measures(ext_angle , val)
    
def perpendicular_bisector(line1 , line2 )  :
    #returns if line1 is perpendicular bisector of line2 
    if PointLiesOnLine(line1[0] , line2).conf > 0.95 :
         out = IsMidpointOf(line1[0] , line2)
    else : 
         out = IsMidpointOf(line1[1] , line2)
         
    ### try this code : out = IsMidpointOf(line1[0] , line2) |  IsMidpointOf(line1[1] , line2)
    return out    
